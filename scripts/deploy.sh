...
gitupdate() {
	ssh $login "cd work/vision2015/ && git checkout src"
...
compile() {
	ssh $login "cd work/vision2015/build*Release/ && make -j4"
}

run() {
	ssh $login "matches-2015/kill.sh"
	sleep 3
	ssh $login "cp work/vision2015/build*Release/vision2015 matches-2015/"
